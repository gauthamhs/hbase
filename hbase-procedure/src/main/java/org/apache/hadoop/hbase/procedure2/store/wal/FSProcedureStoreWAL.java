...
public class FSProcedureStoreWAL implements ProcedureStore {
  private static final Log LOG = LogFactory.getLog(FSProcedureStoreWAL.class);
  ...
  public FSProcedureStoreWAL(final FileSystem fs, final Path logDir) {
    this.fs = fs;
    this.logDir = logDir;
  }
  ...
  public Iterator<Procedure> load() throws IOException {
    ...
    return ProcedureWALFormat.load(it, storeTracker, new ProcedureWALFormat.Loader() {
      ...
      public Procedure deserializeProcedure(long procId, byte[] data) throws IOException {
        return FSProcedureStoreWAL.this.deserializeProcedure(procId, data);
      }
    });
  }
  ...
  private long initOldLogs() throws IOException {
    ...
    for (int i = 0; i < oldLogFiles.length; ++i) {
      ...
      try {
        ...
        String msg = "Unable to read state log " + oldLogFiles[i];
        LOG.error(msg, e);
  ...
  private void syncLoop() throws IOException {
    ...
    while (running.get()) {
      ...
      try {
        ...
        if (slotIndex == 0) {
          LOG.debug("Waiting for data. flushed=" + StringUtils.humanSize(totalSynced));
          waitCond.await();
          if (slotIndex == 0) {
  ...
  protected void serializeProcedure(OutputStream stream, Procedure proc)
      throws IOException {
    Procedure.serializeProcedure(stream, proc);
  }

  protected Procedure deserializeProcedure(long procId, byte[] data)
      throws IOException {
    return Procedure.deserializeProcedure(procId, data);
  }
}