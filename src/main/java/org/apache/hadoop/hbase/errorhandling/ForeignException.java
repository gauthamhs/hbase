...
/**
 * A ForeignException is an exception from a separate thread or process.
 * <p>
...
public class ForeignException extends IOException {
  ...
  /**
   * Name of the throwable's source.  Must be non-null.
   */
  ...
  /**
   * Protobuf byte representation of the exception.  Must be non-null.
   */
  private final byte[] errorInfo;

  /**
   * Did this exception come from via some RPC
   */
  private final boolean fromRpc;
  ...
  private ForeignException(String source, String clazz, ProxyThrowable cause) {
    ...
    assert clazz != null;
    this.source = source;
    this.errorInfo = serialize(source, cause);
    this.clazz = clazz;
    this.fromRpc = true;
  }
  ...
  public ForeignException(String source, String msg) {
    ...
    this.source = source;
    this.errorInfo = serialize(source, getCause());
    this.clazz = getCause().getClass().getName();
    this.fromRpc = false;
  }
   ...
   */
  static class ProxyThrowable extends Throwable {
