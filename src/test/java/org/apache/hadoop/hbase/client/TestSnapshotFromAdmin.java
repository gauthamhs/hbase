...
public class TestSnapshotFromAdmin {
  ...
  public void testBackoffLogic() throws Exception {
    ...
    Mockito
        .when(
          mockMaster.snapshot(
            Mockito.any(SnapshotDescription.class))).thenReturn((long)maxWaitTime);

    // first five times, we return false, last we get success
    Mockito.when(
      mockMaster.isSnapshotDone(
        Mockito.any(SnapshotDescription.class))).thenReturn(false, false,
  ...
  public void testValidateSnapshotName() throws Exception {
    ...
    Mockito.when(
      master.snapshot(Mockito.any(SnapshotDescription.class))).thenReturn((long)0);
    Mockito.when(
      master.isSnapshotDone(
        Mockito.any(SnapshotDescription.class))).thenReturn(true);
