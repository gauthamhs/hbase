...
public class TestProcedureCoordinator {
  ...
  // setup the mocks
  private final ProcedureFactory builder = mock(ProcedureFactory.class);
  ...
  public void resetTest() throws IOException {
    ...
    reset(builder, controller, task, monitor);
  ...
  private ProcedureCoordinator buildNewCoordinator() {
    ...
    return spy(new ProcedureCoordinator(controller, builder, pool));
  }
  ...
  public void testThreadPoolSize() throws Exception {
    ...
    Procedure proc = new Procedure(coordinator,  monitor,
        WAKE_FREQUENCY, TIMEOUT, TIMEOUT_INFO, procName, procData, expected);
    Procedure procSpy = spy(proc);

    Procedure proc2 = new Procedure(coordinator,  monitor,
        WAKE_FREQUENCY, TIMEOUT, TIMEOUT_INFO, procName +"2", procData, expected);
    Procedure procSpy2 = spy(proc2);

    when(builder.createProcedure(eq(coordinator), anyString(), eq(procData),
        eq(expected))).thenReturn(procSpy, procSpy2);
    coordinator.startProcedure(procName, procData, expected);
    ...
    assertNull("Coordinator successfully ran two tasks at once with a single thread pool.",
      coordinator.startProcedure("another op", procData, expected));
  }
  ...
  public void testUnreachableControllerDuringPrepare() throws Exception {
    ...
    Procedure proc = new Procedure(coordinator, WAKE_FREQUENCY,
        TIMEOUT, TIMEOUT_INFO, procName, procData, expected);
    final Procedure procSpy = spy(proc);

    when(builder.createProcedure(eq(coordinator), eq(procName), eq(procData),
        anyListOf(String.class))).thenReturn(procSpy);
    ...
    // run the operation
    proc = coordinator.startProcedure(procName, procData, expected);
    // and wait for it to finish
    proc.waitForCompleted();
    verify(procSpy, atLeastOnce()).receiveError(anyString(), any(ForeignException.class));
  ...
  public void testUnreachableControllerDuringCommit() throws Exception {
    ...
    when(builder.createProcedure(eq(coordinator), eq(procName), eq(procData),
        anyListOf(String.class))).thenReturn(spy);
    ...
    // run the operation
    Procedure task = coordinator.startProcedure(procName, procData, expected);
    // and wait for it to finish
    task.waitForCompleted();
    verify(spy, atLeastOnce()).receiveError(anyString(), any(ForeignException.class));
    verify(coordinator, times(1)).rpcConnectionFailure(anyString(), eq(cause));
  ...
  public void runSimpleProcedure(String... members) throws Exception {
    ...
    Procedure task = new Procedure(coordinator, monitor, WAKE_FREQUENCY,
        TIMEOUT, TIMEOUT_INFO, procName, procData, Arrays.asList(members));
  ...
  public void testEarlyJoiningBarrier() throws Exception {
    ...
    Procedure task = new Procedure(coordinator, monitor, WAKE_FREQUENCY,
        TIMEOUT, TIMEOUT_INFO, procName, procData, Arrays.asList(cohort));
    final Procedure spy = spy(task);
   ...
   * .
   * @param spy Spy on a real {@link Procedure} to be returned by the mock
   *          {@link ProcedureFactory}
  ...
  public void runCoordinatedOperation(Procedure spy, AcquireBarrierAnswer prepareOperation,
    ...
    List<String> expected = Arrays.asList(cohort);
    when(
      builder.createProcedure(eq(coordinator), eq(procName), eq(procData),
        anyListOf(String.class))).thenReturn(spy);
