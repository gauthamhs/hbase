 ...
 */
 ...
 public synchronized  byte[][] rollHLogWriter(String serverName)
   ...
   * In case of failure the snapshot taken will be rolled back.
   *
    ...
    try {
      ...
      try {
        String msg = "Restore snapshot=" + snapshotName +
          " failed. Trying to rollback to snapshot=" + rollbackSnapshot;
        LOG.error(msg, e);
  ...
  private void internalRestoreSnapshot(final String snapshotName, final String tableName)
    ...
    builder.setTable(tableName);
    builder.setName(snapshotName);
    internalRestoreSnapshot(builder.build());
  }
   ...
   * @throws IllegalArgumentException if the restore request is formatted incorrectly
   */
  private void internalRestoreSnapshot(final SnapshotDescription snapshot)
      throws IOException, RestoreSnapshotException {
