...
/**
 * Split snapshot logs and places to recovered.edits
 */
class SnapshotLogSplitter {
   ...
   *
   * @param hlogRefPath Snapshot HLog reference path
   */
  public void splitLog(final Path hlogRefPath) throws IOException {
    LOG.debug("Restore log=" + hlogRefPath +
  ...
  /**
   * TODO: It will be better using a stram instead of a path...
   */
  private Path resolveLogReference(final Path hlogRefPath) throws IOException {
    String logName = hlogRefPath.getName().substring(0, hlogRefPath.getName().lastIndexOf('.'));
    String serverName = hlogRefPath.getParent().getName();
    return new HLogLink(conf, serverName, logName).getAvailablePath(fs);
  }
