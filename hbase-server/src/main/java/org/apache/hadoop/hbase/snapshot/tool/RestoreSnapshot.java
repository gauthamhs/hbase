...
public final class RestoreSnapshot extends Configured implements Tool {
  static final Log LOG = LogFactory.getLog(RestoreSnapshot.class);
  ...
  public int run(String[] args) throws Exception {
    ...
    rootDir = FSUtils.getRootDir(fs.getConf());
    ...
    // Restore snapshot
    CatalogTracker catalogTracker = new CatalogTracker(getConf());
    catalogTracker.start();
    try {
      return restoreSnapshot(catalogTracker, noWALs);
    } catch (Exception e) {
      ...
      if (takeSnapshotName != null) {
        ...
        return restoreSnapshot(catalogTracker, false);
      }
    } finally {
      catalogTracker.stop();
    }
  ...
  private int restoreSnapshot(final CatalogTracker catalogTracker,
    ...
    RestoreSnapshotHelper restoreHelper = new RestoreSnapshotHelper(getConf(), fs,
  ...
  private void printUsageAndExit() {
    ...
    System.err.println("  -no-wal-data          Avoid restoring WAL data.");
    System.err.println("");
    System.exit(1);
  }
