...
public class ZKProcedureMemberRpcs implements ProcedureMemberRpcs {
  ...
  private synchronized void startNewSubprocedure(String path) {
    ...
    try {
      ...
      sendMemberAborted(subproc, new ForeignException(iae));
    } catch (IllegalStateException ise) {
      LOG.error("Illegal state exception ", ise);
      sendMemberAborted(subproc, new ForeignException(ise));
    } catch (KeeperException e) {
  ...
  public void sendMemberAborted(Subprocedure sub, ForeignException ee) {
    ...
    try {
      ...
      byte[] errorInfo = ForeignException.serialize(source, ee);
      // first create the znode for the procedure
      int ver = ZKUtil.checkExists(zkController.getWatcher(), procAbortZNode);
      if (ver != -1) {
        // a znode already written.
        // TODO this is still racy
        return;
      }

      ZKUtil.createSetData(zkController.getWatcher(), procAbortZNode, errorInfo);
