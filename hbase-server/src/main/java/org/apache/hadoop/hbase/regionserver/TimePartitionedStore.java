...
import org.apache.hadoop.hbase.regionserver.HStore.StoreFlusherImpl;
 ...
 */
public class TimePartitionedStore implements Store {

  private final HStore primary;
  private final HStore snapshot;
  private final long splitPoint;

  // check if a key-value belongs in the snapshot store
  ...
  // check if the key-value belongs in the future store
  ...
  public TimePartitionedStore(HStore store, long timestamp) throws IOException {
    this.primary = store;
    this.splitPoint = timestamp;
    this.snapshot = HStore.snapshotAndClone(store);
  }
  ...
  public long upsert(Iterable<KeyValue> kvs, long readpoint) throws IOException {
    long ret = 0;
    ret += snapshot.upsert(Iterables.filter(kvs, inSnapshot), readpoint);
    ret += primary.upsert(Iterables.filter(kvs, inFuture), readpoint);
    return ret;
  }
  ...
  private class SnapshotStoreFlusher implements StoreFlusher {
    ...
    /**
     * Get the approximate size of the store being flushed.Only accurate after calling
     * {@link #prepare()}
     * @return the size of the memstore (in bytes) being flushed.
     */
    public AtomicLong getFlushSize() {
      return this.delegate.getFlushSize();
    }
  }
  ...
  public void bulkLoadHFile(String srcPathStr, long sequenceId) throws IOException {
    primary.bulkLoadHFile(srcPathStr, sequenceId);
  }

  @Override
  public String getColumnFamilyName() {
    return primary.getColumnFamilyName();
  }
