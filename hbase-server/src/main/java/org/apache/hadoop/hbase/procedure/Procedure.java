...
public class Procedure implements Callable<Void>, ForeignExceptionListener {
  ...
  final public Void call() {
    LOG.debug("Starting procedure '" + procName + "'");
    // start the timer
    ...
    try {
      ...
      monitor.rethrowException();
      LOG.debug("Procedure '" + procName + "' execution completed");
    } catch (Exception e) {
      ...
      LOG.error(msg, e);
      receiveError(msg, new ForeignException(e));
    } finally {
      LOG.debug("Running finish phase.");
      sendGlobalBarrierReset();
  ...
  public void sendGlobalBarrierStart() throws ForeignException {
    ...
    try {
      ...
      throw new ForeignException(e, new byte[0]);
    }
  }
