...
public class ProtobufProcedureStoreWAL extends BaseProcedureStoreWAL {
  ...
  protected void serializeProcedure(final OutputStream out, final Procedure proc)
    ...
    if (proc.hasException()) {
      ForeignException exception = proc.getException();
      builder.setException(ForeignException.toProto(exception.getSource(), exception.getCause()));
    }
  ...
  private Procedure deserializeProcedure(long procId, InputStream stream) throws IOException {
    ...
    Procedure proc;
    try {
      Class<?> clazz = Class.forName(proto.getClassName());
      Constructor<?> ctor = clazz.getConstructor();
      proc = (Procedure)ctor.newInstance();
    } catch (Exception e) {
      LOG.error("The procedure class " + proto.getClassName() +
    ...
    if (proto.hasException()) {
      ...
      proc.setFailure(ForeignException.fromProto(proto.getException()));
    }
