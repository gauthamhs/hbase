...
public class GlobalSnapshotOperation extends SnapshotOperation {
  ...
  public void prepare() throws DistributedCommitException {
    ...
    try {
      ...
      taskManager.submitTask(new ReferenceServerWALsTask(snapshot, this.snapshotErrorListener, 
          this.log.getDir(), conf, fs));

      // 4. Wait for the regions to block and the wal to complete or an error
      waitUntilDone();
