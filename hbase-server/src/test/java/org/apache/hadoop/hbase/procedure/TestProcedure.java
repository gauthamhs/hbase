...
public class TestProcedure {
  ...
  class LatchedProcedure extends Procedure {
    ...
    public LatchedProcedure(ProcedureCoordinator coord, ForeignExceptionDispatcher monitor,
        long wakeFreq, long timeout, Object[] timeoutInfo, String opName, byte[] data,
        List<String> expectedMembers) {
      super(coord, monitor, wakeFreq, timeout, timeoutInfo, opName, data, expectedMembers);
    }
    ...
    @Override
    public void sendGlobalBarrierReset() {
  ...
  public void testSingleMember() throws Exception {
    ...
    LatchedProcedure proc = new LatchedProcedure(coord, new ForeignExceptionDispatcher(), 100,
        Integer.MAX_VALUE, null, "op", null, members);
    ...
    verify(procspy, never()).sendGlobalBarrierReset();
    ...
    verify(procspy).sendGlobalBarrierReset();
    verify(procspy, never()).receiveError(anyString(), any(ForeignException.class));
  }
  ...
  public void testErrorPropagation() throws Exception {
    ...
    ForeignException cause = new ForeignException("External Exception");
    proc.receiveError("test before anything starts", cause);
  ...
  public void testBarrieredErrorPropagation() throws Exception {
    ...
    procspy.startedAcquireBarrier.await();
    ForeignException cause = new ForeignException("External Exception");
    procspy.receiveError("test after prepare", cause);
    procspy.barrierAcquiredByMember(members.get(0));
    t.join();
