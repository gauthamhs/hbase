...
import org.apache.hadoop.hbase.errorhandling.TimeoutForeignException;
...
public class TestZKProcedure {
  ...
  private void runCommit(String... members) throws Exception {
    ...
    ProcedureFactory procBuilder = Mockito.spy(new ProcedureFactory(WAKE_FREQUENCY, TIMEOUT, TIMEOUT_INFO));
    ...
    ProcedureCoordinator coordinator = new ProcedureCoordinator(coordinatorComms, procBuilder, pool);

    // build and start members
    // NOTE: There is a single subprocedure builder for all members here.
    SubprocedureFactory subprocBuilder = Mockito.mock(SubprocedureFactory.class);
    ...
    for (String member : members) {
      ...
      ProcedureMember procMember = new ProcedureMember(comms, pool2, subprocBuilder);
    ...
    Mockito.when(subprocBuilder.buildSubprocedure(Mockito.eq(opName),
    ...
    Mockito.when(procBuilder.createProcedure(coordinator, opName, data, expected)).thenReturn(proc);

    // start running the operation
    Procedure task = coordinator.startProcedure(opName, data, expected);
    ...
    verifyCohortSuccessful(expected, subprocBuilder, subprocs, once, once, never(), once, false);
   ...
   */
  @Ignore // TODO This hangs still need to address.
  @Test
  public void testMultiCohortWithMemberTimeoutDuringPrepare() throws Exception {
    ...
    ProcedureFactory coordinatorTaskBuilder = Mockito.spy(new ProcedureFactory(
    ...
    ThreadPoolExecutor pool = ProcedureCoordinator.defaultPool(COORDINATOR_NODE_NAME, KEEP_ALIVE, POOL_SIZE, WAKE_FREQUENCY);
    ProcedureCoordinator coordinator = new ProcedureCoordinator(coordinatorController, coordinatorTaskBuilder, pool);
    ...
    for (String member : expected) {
      ...
      ThreadPoolExecutor pool2 = ProcedureMember.defaultPool(WAKE_FREQUENCY, KEEP_ALIVE, 1, member);
      ProcedureMember manager = new ProcedureMember(controller, pool2, subprocBuilder);
      members.add(new Pair<ProcedureMember, ZKProcedureMemberRpcs>(manager, controller));
      ...
      Mockito.doAnswer(new Answer<Void>() {
        ...
        public Void answer(InvocationOnMock invocation) throws Throwable {
          ...
          if (index == memberErrorIndex) {
            ...
            ForeignException remoteCause = new TimeoutForeignException("subprocTimeout" , 1, 2, 0);
            Subprocedure r = ((Subprocedure) invocation.getMock());
            LOG.error("Remote commit failure, not propagating error:" + remoteCause);
            r.monitor.receiveError("Remote failure", remoteCause);
            ...
            try {
              ForeignExceptionDispatcher.waitForLatch(coordinatorReceivedErrorLatch, new ForeignExceptionSnare(),
    ...
    Mockito.when(
      subprocBuilder.buildSubprocedure(Mockito.eq(opName),
        (byte[]) Mockito.argThat(new ArrayEquals(data)))).thenAnswer(
      new Answer<Subprocedure>() {
        @Override
        public Subprocedure answer(InvocationOnMock invocation) throws Throwable {
    ...
    Procedure coordinatorTask = Mockito.spy(new Procedure(coordinator,
        coordinatorTaskErrorMonitor, WAKE_FREQUENCY, TIMEOUT, TIMEOUT_INFO,
        opName, data, expected));
    Mockito.when(coordinatorTaskBuilder.createProcedure(coordinator, opName, data, expected))
    ...
    }).when(coordinatorTask).receiveError(Mockito.anyString(),
        Mockito.any(ForeignException.class), Mockito.anyObject());
    ...
    verifyCohortSuccessful(expected, subprocBuilder, cohortTasks, once, never(), once,
      once, true);
   ...
   */
  private void waitAndVerifyProc(Procedure op, VerificationMode prepare,
    ...
    Mockito.verify(op, prepare).sendGlobalBarrierStart();
    Mockito.verify(op, commit).sendGlobalBarrierReached();
    Mockito.verify(op, finish).sendGlobalBarrierReset();
    assertEquals("Operation error state was unexpected", opHasError, op.getErrorMonitor()
        .hasException());
  }
  ...
  private void verifyCohortSuccessful(List<String> cohortNames,
      SubprocedureFactory cohortMemberBuilder, Iterable<Subprocedure> cohortTasks,
    ...
    Mockito.verify(cohortMemberBuilder, Mockito.times(cohortNames.size())).buildSubprocedure(
