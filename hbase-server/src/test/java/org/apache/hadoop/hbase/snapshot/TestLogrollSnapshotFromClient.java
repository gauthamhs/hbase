...
public class TestLogrollSnapshotFromClient {
  private static final Log LOG = LogFactory.getLog(TestLogrollSnapshotFromClient.class);
  ...
  private static void setupConf(Configuration conf) {
    ...
    // setup the cleaners
    conf.set(HFileCleaner.MASTER_HFILE_CLEANER_PLUGINS, "org.apache.hadoop.hbase.master.snapshot.SnapshotHFileCleaner");
    conf.set(HConstants.HBASE_MASTER_LOGCLEANER_PLUGINS, "org.apache.hadoop.hbase.master.snapshot.SnapshotLogCleaner");
  }
  ...
  /**
   * Test logroll snapshotting a table that is online 
   * @throws Exception
   */
  @Test
  public void testLogrollTableSnapshot() throws Exception {
    ...
    admin.snapshot(snapshotString, STRING_TABLE_NAME, SnapshotDescription.Type.LOGROLL);
  ...
  public void testSnapshotFailsOnNonExistantTable() throws Exception {
    ...
    try {
      admin.snapshot("fail", tableName, SnapshotDescription.Type.LOGROLL);
  ...
  @Test(timeout = 15000)
  public void testAsyncLogrollSnapshot() throws Exception {
    HBaseAdmin admin = UTIL.getHBaseAdmin();
    SnapshotDescription snapshot = SnapshotDescription.newBuilder().setName("asyncSnapshot")
        .setTable(STRING_TABLE_NAME).setType(SnapshotDescription.Type.LOGROLL).build();
  ...
  /**
   * Basic end-to-end test of logroll based snapshots
   */
  @Test
  public void testLogrollCreateListDestroy() throws Exception {
    ...
    String snapshotName = "logrollSnapshotCreateListDestroy";
    // test creating the snapshot
    admin.snapshot(snapshotName, STRING_TABLE_NAME, SnapshotDescription.Type.LOGROLL);
    ...
    // make sure we don't have any snapshots
    SnapshotTestingUtils.assertNoSnapshots(admin);
    LOG.debug("------- Snapshot Logroll Create List Destroy-------------");
